...
def run(ctx: click.Context, source_dir: str, test_dir: Optional[str], 
        output: str, test_filter: Optional[str], ignore: List[str],
        omit: List[str], formulas: List[str], config: str) -> None:
    """
    ...
    try:
        ...
        intermediate_file = output.replace('.json', '_coverage.json') if output != 'report.json' else 'coverage.json'
        ...
        console.print("[bold green]Running complete fault localization pipeline...[/bold green]")
        console.print(f"Source dir: [cyan]{test_config.source_dir}[/cyan]")
        if test_config.test_dir:
            console.print(f"Test dir: [cyan]{test_config.test_dir}[/cyan]")
        ...
        if not result.failed_tests:
            ...
            console.print(f"[bold green]✓[/bold green] Pipeline completed successfully!")
