...
class TestRunCommandDocumentation:
    ...
    def test_run_command_help_completeness(self):
        ...
        assert 'Run complete fault localization pipeline' in result.output
        assert 'Executes tests with coverage collection and calculates fault localization' in result.output
        ...
        assert 'report.json)' in result.output  # output default (may be on multiple lines)
    ...
    def test_run_command_option_consistency(self):
        ...
        # From test command
        test_options = ['--source-dir', '--test-dir', '--test-filter', '--ignore', '--omit']
