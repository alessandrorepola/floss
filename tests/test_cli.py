...
"""
    ...
    def test_run_command_help(self):
        ...
        assert (
            "default: current directory" in result.output or "default:" in result.output
        )
...
""")
    ...
    def test_run_command_default_output_directory(self):
        ...
        try:
            ...
            (self.temp_dir / "test_simple.py").write_text(
            ...
            with patch("src.pyfault.cli.main.FaultLocalizer") as mock_localizer_class:
                ...
                assert "pyfault_report" in str(output_dir)
...
"""
    ...
    def test_fl_command_nonexistent_coverage_file(self):
        ...
        result = self.runner.invoke(
            main, ["fl", "--coverage-file", "/nonexistent/coverage.csv"]
        )
        assert result.exit_code != 0
        assert "not found" in result.output.lower() or "error" in result.output.lower()
    ...
    @patch("src.pyfault.cli.main.FaultLocalizer")
    def test_fl_command_basic_execution(
        self, mock_localizer_class, mock_csv_reporter_class
    ):
        ...
        result = self.runner.invoke(
            main, ["fl", "--coverage-file", str(self.coverage_file)]
        )
...
""")
            ...
            with patch(
                "src.pyfault.cli.main.CoverageCollector"
            ) as mock_coverage_collector:
                with patch("src.pyfault.cli.main.PytestRunner") as mock_pytest_runner:
                    with patch(
                        "src.pyfault.cli.main.CoverageMatrix"
                    ) as mock_coverage_matrix:
                        with patch(
                            "src.pyfault.cli.main.CSVReporter"
                        ) as mock_csv_reporter:
                            # Setup mocks
                            mock_collector = Mock()
                            ...
                            mock_coverage_matrix.from_test_results.return_value = (
                                mock_matrix
                            )
    ...
    @patch("src.pyfault.cli.main.CSVReporter")
    def test_test_command_basic_execution(
        self,
        mock_csv_reporter,
        mock_coverage_matrix,
        mock_pytest_runner,
        mock_coverage_collector,
    ):
...
class TestCLIErrorHandling:
    ...
    def test_invalid_formula_name(self):
        ...
        coverage_file.write_text(
            "Element,File,Line,ElementType,ElementName\nOUTCOME,,,,,"
        )
    ...
    def test_fault_localizer_exception(self, mock_localizer_class):
        ...
        result = self.runner.invoke(
            main, ["run", "--source-dir", str(source_dir), "--test-dir", str(test_dir)]
        )
...
class TestCLIOutputFormatting:
    ...
    @patch("src.pyfault.cli.main.CSVReporter")
    @patch("src.pyfault.cli.main.FaultLocalizer")
    def test_results_display_formatting(
        self, mock_localizer_class, mock_csv_reporter_class
    ):
...
""")
        ...
        result = self.runner.invoke(
            main, ["fl", "--coverage-file", str(coverage_file), "--top", "5"]
        )
        ...
        assert (
            "Analysis complete" in result.output or "complete" in result.output.lower()
        )
...
""")
    ...
    def test_end_to_end_fl_command(self):
        ...
        assert (
            "Loading coverage data" in result.output
    ...
    def test_error_handling_with_real_files(self):
        ...
        result = self.runner.invoke(main, ["fl", "--coverage-file", str(bad_coverage)])
