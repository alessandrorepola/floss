...
def create_top_n_comparison():
    ...
    ax.set_title('SBFL Formula Effectiveness Comparison - Top-N Performance', fontsize=16, fontweight='bold')
...
def create_discrimination_comparison():
    ...
    metrics = ['auc_roc', 'score_separation', 'overlap_ratio']
    metric_labels = ['AUC-ROC', 'Separazione Punteggi', 'Overlap Ratio']
    ...
    plt.suptitle('Capacità Discriminante delle Formule SBFL', fontsize=16, fontweight='bold')
    plt.tight_layout()
...
def create_rank_distribution():
    ...
    ax.set_title('Distribuzione dei Rank per Formula SBFL', fontsize=16, fontweight='bold')
...
def create_project_performance():
    ...
    # Success rate
    bars1 = ax1.bar(df['Progetto'], df['Success_Rate'], 
    ...
    # Numero di bug
    bars2 = ax2.bar(df['Progetto'], df['Analyzable_Bugs'], 
                    color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4'], alpha=0.7, label='Analizzabili')
    bars3 = ax2.bar(df['Progetto'], df['Total_Bugs'] - df['Analyzable_Bugs'], 
                    bottom=df['Analyzable_Bugs'], color='gray', alpha=0.5, label='Non Analizzabili')
    ...
    for i, (bar, total) in enumerate(zip(bars2, df['Total_Bugs'])):
        ax2.text(bar.get_x() + bar.get_width()/2, total + 0.2, 
...
def create_summary_radar():
    ...
    for formula in formulas:
        ...
        avg_rank_norm = max(0, 1 - (eff_data['avg_rank'] - 100) / 300)  # normalizza rank
        
        metrics.append([top10_norm, auc_norm, sep_norm, overlap_norm, avg_rank_norm])
    
    # Setup radar chart
    categories = ['Top-10\nRate', 'AUC-ROC', 'Separazione\nPunteggi', 'Basso\nOverlap', 'Buon\nRank']
    ...
    for i, (formula, metric_values) in enumerate(zip(formulas, metrics)):
        ...
        ax.plot(angles, values, 'o-', linewidth=2, label=formula.capitalize(), color=colors[i])
    ...
    ax.set_ylim(0, 1)
    ax.set_title('Multi-dimensional SBFL Formula Comparison', 
