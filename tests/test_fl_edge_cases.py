    ...
    def test_large_number_of_tests(self):
        ...
        coverage_json = {
            ...
            "files": {
                "file.py": {
                    "contexts": {
                        "1": [f"{test}|run" for test in passed_tests[:10]],  # 10 passing tests
                        "2": [f"{test}|run" for test in failed_tests[:5]]    # 5 failing tests
                    }
                }
            }
        }
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as input_file:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as output_file:
                try:
                    ...
                    assert result["fl_metadata"]["total_lines_analyzed"] == num_files * 2
