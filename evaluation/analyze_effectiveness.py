    ...
    for project_dir in examples_dir.iterdir():
        ...
        if project_name == "fastapi":
            ...
            if multi_bugs_dir.exists() and (multi_bugs_dir / "report.json").exists():
                bug_info = extract_multi_bug_info(project_name, multi_bugs_dir, "multi-1-9-12-13-15-16")
                if bug_info:
                    bugs.append(bug_info)
...
def extract_bug_info(project: str, bug_dir: Path, bug_id: Optional[str] = None) -> Optional[BugInfo]:
...
def parse_patch_file(patch_path: Path) -> Tuple[List[str], List[Tuple[str, int]]]:
    ...
    try:
        ...
        hunks = re.finditer(hunk_pattern, content)
            ...
            if line.startswith('diff --git'):
                ...
                if match:
                    ...
                    while j < len(lines) and not lines[j].startswith('@@') and not lines[j].startswith('diff'):
                        ...
                        # Incrementa il numero di linea solo per le linee che esistevano nel file originale
...
def extract_multi_bug_info(project: str, multi_bug_dir: Path, bug_id: Optional[str] = None) -> Optional[BugInfo]:
...
def analyze_report_effectiveness(bug_info: BugInfo) -> Optional[EffectivenessMetrics]:
            ...
            if sl['file'] == buggy_file and sl['line'] == buggy_line:
                ...
                for formula in formulas:
                    if formula in rankings:
                        rank = find_line_rank(rankings[formula], buggy_file, buggy_line, formula)
                        if rank is not None:
    ...
    return EffectivenessMetrics(
        ...
        best_rank=best_rank,
        avg_rank=avg_rank,
        ochiai_best_rank=min(formula_ranks['ochiai']) if formula_ranks['ochiai'] else None,
        tarantula_best_rank=min(formula_ranks['tarantula']) if formula_ranks['tarantula'] else None,
        jaccard_best_rank=min(formula_ranks['jaccard']) if formula_ranks['jaccard'] else None,
        dstar2_best_rank=min(formula_ranks['dstar2']) if formula_ranks['dstar2'] else None
    )


def find_line_rank(ranked_lines: List[Dict], target_file: str, target_line: int, formula: str) -> Optional[int]:
...
def generate_summary_report(metrics_list: List[EffectivenessMetrics]) -> Dict:
    ...
    if analyzable_bugs > 0:
        summary["overall_effectiveness"] = {
            "top_1_rate": sum(1 for m in bugs_with_patches if m.top_1_hit) / analyzable_bugs,
            "top_3_rate": sum(1 for m in bugs_with_patches if m.top_3_hit) / analyzable_bugs,
            "top_5_rate": sum(1 for m in bugs_with_patches if m.top_5_hit) / analyzable_bugs,
            "top_10_rate": sum(1 for m in bugs_with_patches if m.top_10_hit) / analyzable_bugs,
    ...
    for formula_name, rank_attr in formulas.items():
        ...
        formula_bugs = [m for m in bugs_with_patches if getattr(m, rank_attr) is not None]
...
def save_results(metrics_list: List[EffectivenessMetrics], summary: Dict):
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow([
            "Project", "Bug_ID", "Total_Lines_Analyzed", "Buggy_Lines_Found",
            "Top_1_Hit", "Top_3_Hit", "Top_5_Hit", "Top_10_Hit",
            "Best_Rank", "Avg_Rank", 
            "Ochiai_Best_Rank", "Tarantula_Best_Rank", 
            "Jaccard_Best_Rank", "Dstar2_Best_Rank"
        ])
        
        for metric in metrics_list:
            writer.writerow([
                metric.project, metric.bug_id, metric.total_lines_analyzed,
                metric.buggy_lines_found, metric.top_1_hit, metric.top_3_hit,
                metric.top_5_hit, metric.top_10_hit, metric.best_rank, metric.avg_rank,
                metric.ochiai_best_rank, metric.tarantula_best_rank,
                metric.jaccard_best_rank, metric.dstar2_best_rank
            ])
...
def main():
    ...
    for project, data in summary["bugs_by_project"].items():
        if data["analyzable"] > 0:
            print(f"  {project}: {data['top_10_hits']}/{data['analyzable']} bugs in top-10 "
    ...
    if "formula_effectiveness" in summary:
        ...
        print(f"{'Formula':<12} {'Bug':<4} {'Top-1':<6} {'Top-3':<6} {'Top-5':<6} {'Top-10':<6} {'Avg':<6} {'Med':<5} {'Best':<5} {'P25':<4} {'P75':<4}")
        print("-" * 70)
        
        for formula, data in summary["formula_effectiveness"].items():
            if data["bugs_analyzed"] > 0:
                print(f"{formula.capitalize():<12} "
        ...
        print("-" * 30)
        best_top1 = max(summary["formula_effectiveness"].items(), 
                       key=lambda x: x[1].get("top_1_rate", 0) if x[1]["bugs_analyzed"] > 0 else 0)
        best_top10 = max(summary["formula_effectiveness"].items(), 
                        key=lambda x: x[1].get("top_10_rate", 0) if x[1]["bugs_analyzed"] > 0 else 0)
        best_avg = min(summary["formula_effectiveness"].items(), 
                      key=lambda x: x[1].get("avg_rank", float('inf')) if x[1]["bugs_analyzed"] > 0 else float('inf'))
        
        print(f"Top-1 rate: {best_top1[0].capitalize()} ({best_top1[1]['top_1_rate']:.1%})")
        print(f"Top-10 rate: {best_top10[0].capitalize()} ({best_top10[1]['top_10_rate']:.1%})")
        print(f"Average rank: {best_avg[0].capitalize()} ({best_avg[1]['avg_rank']:.1f})")
