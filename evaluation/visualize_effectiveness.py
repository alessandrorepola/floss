...
def create_effectiveness_charts(df, summary):
    ...
    fig.suptitle('PyFault: Fault Localization Effectiveness Analysis', fontsize=16, fontweight='bold')
    
    # 1. Top-N Hit Rates (Overall)
    ax1 = axes[0, 0]
    eff = summary["overall_effectiveness"]
    top_n_rates = [eff["top_1_rate"], eff["top_3_rate"], eff["top_5_rate"], eff["top_10_rate"]]
    top_n_labels = ["Top-1", "Top-3", "Top-5", "Top-10"]
    
    bars1 = ax1.bar(top_n_labels, [rate * 100 for rate in top_n_rates], 
    ...
    ax2.bar(x - 1.5*width, projects_df['Top-1'], width, label='Top-1', color='#FF6B6B')
    ax2.bar(x - 0.5*width, projects_df['Top-3'], width, label='Top-3', color='#4ECDC4')
    ax2.bar(x + 0.5*width, projects_df['Top-5'], width, label='Top-5', color='#45B7D1')
    ax2.bar(x + 1.5*width, projects_df['Top-10'], width, label='Top-10', color='#96CEB4')
    
    ax2.set_xlabel('Project')
    ax2.set_ylabel('Success Rate (%)')
    ax2.set_title('Effectiveness by Project')
    ax2.set_xticks(x)
    ax2.set_xticklabels([f"{row['Project']}\\n({row['Analyzable']} bugs)" 
    ...
    if len(df_with_ranks) > 0:
        ...
        ax3.axvline(np.median(ranks), color='red', linestyle='--', 
                   label=f'Mediana: {np.median(ranks):.0f}')
        ax3.axvline(np.mean(ranks), color='orange', linestyle='--', 
    ...
    formula_cols = ['Ochiai_Best_Rank', 'Tarantula_Best_Rank', 'Jaccard_Best_Rank', 'Dstar2_Best_Rank']
...
def create_detailed_table(df):
    ...
    display_df = df[['Project', 'Bug_ID', 'Total_Lines_Analyzed', 'Buggy_Lines_Found',
                     'Top_1_Hit', 'Top_3_Hit', 'Top_5_Hit', 'Top_10_Hit', 'Best_Rank']].copy()
    
    # Rename columns for display
    display_df.columns = ['Project', 'Bug ID', 'Lines Analyzed', 'Buggy Lines Found',
                         'Top-1', 'Top-3', 'Top-5', 'Top-10', 'Best Rank']
...
def generate_summary_statistics(df, summary):
    ...
    if analyzable_bugs > 0:
        ...
        print(f"   • Top-1 hit rate: {eff['top_1_rate']:.1%} ({int(eff['top_1_rate'] * analyzable_bugs)}/{analyzable_bugs})")
        print(f"   • Top-3 hit rate: {eff['top_3_rate']:.1%} ({int(eff['top_3_rate'] * analyzable_bugs)}/{analyzable_bugs})")
        print(f"   • Top-5 hit rate: {eff['top_5_rate']:.1%} ({int(eff['top_5_rate'] * analyzable_bugs)}/{analyzable_bugs})")
        print(f"   • Top-10 hit rate: {eff['top_10_rate']:.1%} ({int(eff['top_10_rate'] * analyzable_bugs)}/{analyzable_bugs})")
    ...
    for project, data in summary["bugs_by_project"].items():
        if data["analyzable"] > 0:
            ...
            print(f"     - Top-10 success: {data['top_10_hits']}/{data['analyzable']} ({data['top_10_hits']/data['analyzable']:.1%})")
