...
def analyze_formula_discrimination(report_path: str, buggy_lines: List[Tuple[str, int]]) -> List[DiscriminationMetrics]:
...
def calculate_discrimination_metrics(formula: str, buggy_scores: List[float], 
                                   non_buggy_scores: List[float]) -> DiscriminationMetrics:
    ...
    if overlap_start <= overlap_end:
        ...
        overlap_buggy = sum(1 for score in buggy_scores if overlap_start <= score <= overlap_end)
        overlap_non_buggy = sum(1 for score in non_buggy_scores if overlap_start <= score <= overlap_end)
        overlap_ratio = (overlap_buggy + overlap_non_buggy) / (len(buggy_scores) + len(non_buggy_scores))
    else:
        overlap_ratio = 0.0
...
def generate_discrimination_report(all_metrics: List[DiscriminationMetrics]) -> Dict:
    ...
    for metric_name in ["score_separation", "overlap_ratio", "auc_roc", "perfect_separation_rate"]:
        comparison[metric_name] = {}
        for formula in formulas:
            if metric_name == "perfect_separation_rate":
                comparison[metric_name][formula] = report["discrimination_analysis"][formula][metric_name]
            else:
                comparison[metric_name][formula] = report["discrimination_analysis"][formula][metric_name]["mean"]
    ...
    if formulas:  # Solo se ci sono formule da confrontare
        ...
        best_perfect_sep = max(formulas, key=lambda f: comparison["perfect_separation_rate"][f])
...
def main():
    ...
    if not os.path.exists(csv_path):
        print(f"Errore: File {csv_path} non trovato. Esegui prima analyze_effectiveness.py")
        return
    
    # Carica i bug e le loro informazioni
    from analyze_effectiveness import find_all_bugs, parse_patch_file
    ...
    for bug in bugs:
        ...
        metrics = analyze_formula_discrimination(bug.report_path, normalized_buggy_lines)
    ...
    print(f"Best score separation: {summary['best_score_separation']['formula'].capitalize()} "
          f"({summary['best_score_separation']['value']:.3f})")
    print(f"Best AUC-ROC: {summary['best_auc_roc']['formula'].capitalize()} "
          f"({summary['best_auc_roc']['value']:.3f})")
    print(f"Least overlap: {summary['least_overlap']['formula'].capitalize()} "
          f"({summary['least_overlap']['value']:.3f})")
    print(f"Most perfect separations: {summary['most_perfect_separations']['formula'].capitalize()} "
          f"({summary['most_perfect_separations']['value']:.1%})")
    
    print(f"\nDettaglio per formula:")
    print(f"{'Formula':<12} {'AUC-ROC':<8} {'Separazione':<11} {'Overlap':<8} {'Sep.Perfette':<12}")
    print("-" * 60)
    ...
    for formula in ['ochiai', 'tarantula', 'jaccard', 'dstar2']:
        if formula in comparison.get("auc_roc", {}):
            ...
            print(f"{formula.capitalize():<12} {auc:<8.3f} {sep:<11.3f} {overlap:<8.3f} {perfect:<12.1%}")
